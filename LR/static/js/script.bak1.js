...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const isAgendamentoPage = !!document.getElementById('categoria-select') || !!document.getElementById('categoria-1');
        ...
        if (isConsultaPage) {
            ...
            if (document.getElementById('categoria-1')) {
                ...
                carregarCategorias('categoria-select', [], true);
            }
        }
    ...
    if (agendamentoForm) {
        ...
        if (categoriasContainer) {
            Array.from(categoriasContainer.children).forEach(child => {
        ...
        function getSelectedServiceIds() {
            const legacySelected = legacyServicosContainer
                ? Array.from(legacyServicosContainer.querySelectorAll('input[type="checkbox"]:checked'))
                : [];
            const dynamicSelected = categoriasContainer
                ? Array.from(categoriasContainer.querySelectorAll('input[type="checkbox"]:checked'))
                : [];
            return legacySelected
                .concat(dynamicSelected)
                .map(input => parseInt(input.value, 10))
                .filter(Number.isFinite);
        }

        function carregarCategorias(selectId, excludeCategories = [], isLegacy = false) {
            const categoriaSelect = document.getElementById(selectId);
            const containerSuffix = selectId.split('-')[1];
            const servicosContainer = isLegacy
                ? legacyServicosContainer
                : document.getElementById(`servicos-checkboxes-${containerSuffix}`);

            if (!categoriaSelect || !servicosContainer) {
                console.warn('Select ou container de serviços não encontrado:', selectId);
                return;
            }

            const placeholderText = '-- Escolha uma Categoria --';
            ...
            const handleCategoryChange = async () => {
                ...
                try {
                    const response = await fetch(`/api/servicos?categoria=${encodeURIComponent(categoria)}`);
                    const data = await response.json();
            ...
            async function carregarOpcoes() {
                try {
                    ...
                    categoriaSelect.innerHTML = `<option value="">${placeholderText}</option>`;
                    data.forEach(cat => {
                        if (!excludeCategories.includes(cat) || cat === valorAtual) {
                            const option = document.createElement('option');
                            option.value = cat;
                    ...
                    if (valorAtual && categoriaSelect.querySelector(`option[value="${valorAtual}"]`)) {
            ...
            if (!servicosContainer.dataset.changeListenerAttached) {
                servicosContainer.addEventListener('change', onServicesChange);
                servicosContainer.dataset.changeListenerAttached = 'true';
            }
        }

        if (addCategoriaBtn && categoriasContainer) {
            addCategoriaBtn.addEventListener('click', () => {
                categoriaCount += 1;
                const index = categoriaCount;
                const newGroup = document.createElement('div');
                newGroup.classList.add('form-group', 'mb-3');
                newGroup.innerHTML = `
                    <label for="categoria-${index}">Adicione mais serviços (opcional):</label>
                    <select class="form-select categoria" id="categoria-${index}">
                        <option value="">-- Escolha uma Categoria --</option>
                    </select>
                    <div id="servicos-checkboxes-${index}" class="servicos-checkboxes"></div>
                `;
                categoriasContainer.appendChild(newGroup);
                const selectedCategories = [
                    legacyCategoriaSelect ? legacyCategoriaSelect.value : null,
                    ...Array.from(categoriasContainer.querySelectorAll('.categoria')).map(sel => sel.value)
                ].filter(Boolean);
                carregarCategorias(`categoria-${index}`, selectedCategories);
            });
        }
        ...
        const onServicesChange = () => {
            ...
            if (selectedIds.length > 0) {
                ...
                .then(data => {
                    if (data.success) {
                        totalInfo.textContent = `Total: Preço R$ ${data.preco_total.toFixed(2)}`;
        ...
        if (legacyServicosContainer) {
            legacyServicosContainer.addEventListener('change', onServicesChange);
        }
        function carregarProfissionaisParaAgendamento(idServicosSelecionados) {
            ...
            fetch('/api/profissionais_por_servicos', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id_servicos: idServicosSelecionados })
            })
            .then(res => res.json())
            .then(profissionais => {
                ...
                // Limpar e adicionar sem duplicatas
                profissionalSelect.innerHTML = '<option value="">-- Escolha um Profissional --</option>';
                const seenIds = new Set();
                profissionais.forEach(prof => {
                    if (!seenIds.has(prof.id_profissional)) {
                        const option = document.createElement('option');
                        option.value = prof.id_profissional;
                        option.textContent = prof.nome;
                        profissionalSelect.appendChild(option);
                        seenIds.add(prof.id_profissional);
                    }
                });
        ...
        function carregarHorarios(data) {
            ...
            if (!idProfissional || selectedIds.length === 0 || !data) {
                return;
            }
        ...
        horaSelect.addEventListener('change', () => {
            const hasHora = Boolean(horaSelect.value);
            submitBtn.disabled = !hasHora;
            if (observacoesGroup) {
                observacoesGroup.style.display = hasHora ? 'block' : 'none';
            }
        });
